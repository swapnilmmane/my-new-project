...
class ApiGateway {
    ...
    constructor({ routes }: { routes: Array<any> }) {
        this.api = new aws.apigatewayv2.Api("my-full-stack-application", {
            protocolType: "HTTP",
            description: "My Full Stack Application - API Gateway"
        });
        ...
        for (let i = 0; i < routes.length; i++) {
            ...
            const integration = new aws.apigatewayv2.Integration(route.name, {
                description: "My Full Stack Application - GraphQL API Integration",
                apiId: this.api.id,
